...
- **Proxy Security**: Optional shared-secret authentication for the proxy data plane. Set it with `cc-switch proxy auth-token <TOKEN>` (or `--clear`); once configured the proxy rejects requests without a matching `x-api-key`/`Authorization` header, takeover projection writes the real token into live app configs (instead of the `PROXY_MANAGED` placeholder), and listening on a non-loopback address refuses to start until a token exists. Default remains off (loopback deployments unchanged).
